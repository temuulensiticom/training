    ...
    public class DatabaseService
    {
        ...
        public async Task<ExecutionResult> ExecuteQueriesAsync(string sqlContent, bool executeOnDb1 = true, bool executeOnDb2 = true)
        {
            ...
            try
            {
                ...
                if (executeOnDb1)
                {
                    var db1Result = await ExecuteQueriesOnDatabase(_connectionString1, queries, "Database 1 (Port 3308)");
                ...
                if (executeOnDb2)
                {
                    var db2Result = await ExecuteQueriesOnDatabase(_connectionString2, queries, "Database 2 (Port 3309)");
        ...
        private List<string> ParseSqlQueries(string sqlContent)
        {
            ...
            var objectType = ""; // PROCEDURE, FUNCTION, TRIGGER, EVENT
            ...
            foreach (var line in lines)
            {
                ...
                {
                    inStoredObject = true;
                    if (upperLine.Contains("PROCEDURE")) objectType = "PROCEDURE";
                    else if (upperLine.Contains("FUNCTION")) objectType = "FUNCTION";
                    else if (upperLine.Contains("TRIGGER")) objectType = "TRIGGER";
                    else if (upperLine.Contains("EVENT")) objectType = "EVENT";
                }
